...
public class Validator2 {
    ...
    public Validator2(Relation relation) {
        ...
        // Pre-seed: unique column determines all others
        for (int lhsAttr = 0; lhsAttr < numAttributes; lhsAttr++) {
            ...
            for (int rhsAttr = 0; rhsAttr < numAttributes; rhsAttr++) {
                ...
                if (relation.getUnaryPLIs()[rhsAttr].isConstant()){
                    continue;
                }
    ...
    public List<IntIntImmutablePair> validate(FDSet newNonFds, int targetLevel) {
        ...
        // Only process currentLevel, not the entire while loop
        Map<BitSet, FDTreeNode> levelNodes = new LinkedHashMap<>();
        collectLevel(posCover.root, new BitSet(numAttributes), 0, currentLevel, levelNodes);

        if (levelNodes.isEmpty()) {
            currentLevel++;
            return currentLevel <= targetLevel ? new ArrayList<>() : null;
        }
        ...
        for (Map.Entry<BitSet, FDTreeNode> entry : levelNodes.entrySet()) {
            ...
            FDTreeNode node = entry.getValue();
            BitSet rhss = (BitSet) node.fds.clone();
            if (rhss.isEmpty()) {
                continue;
            }

            if (currentLevel == 0) {
                for (int rhs = rhss.nextSetBit(0); rhs >= 0; rhs = rhss.nextSetBit(rhs + 1)) {
                ...
                for (int rhs = invalid.nextSetBit(0); rhs >= 0; rhs = invalid.nextSetBit(rhs + 1)) {
                    invalidLhs.add(lhs);
                    invalidRhs.add(rhs);
                    numInvalid++;
                }
        ...
        // Mirror original: if at targetLevel, stop here — don't specialize further
        if (currentLevel >= targetLevel) {
            currentLevel++;
            return null;
        }

        // Only specialize if below targetLevel
        ...
        // Go back to sampling if inefficient
        if (numInvalid > numValid * THRESHOLD) {
            return suggestions;
        }

        // Current level done — advance and keep sampling for next level
        currentLevel++;
        return suggestions.isEmpty() ? new ArrayList<>() : suggestions;
    }

    // Returns only FDs whose LHS cardinality == level
    public Map<Integer, List<BitSet>> results(int level) {
        Map<Integer, List<BitSet>> out = new HashMap<>();
        collectResults(posCover.root, new BitSet(numAttributes), level, 0, out);
        return out;
    }
    ...
    private void specialize(BitSet lhs, int rhs) {
        ...
        collectGeneralizations(posCover.root, lhs, rhs, new BitSet(numAttributes), generals);
...
//                else {
//                    if (rhs == 2) {
//                        System.out.println("Blocked: " + gen.clone() + " -> 2" + " | violation lhs was: " + lhs);
//                    }
//                }
                gen.clear(attr);
            }
    ...
    private void collectLevel(FDTreeNode node, BitSet currentLhs, int currentDepth, int targetDepth, Map<BitSet, FDTreeNode> out) {
        ...
        for (int attr = 0; attr < numAttributes; attr++) {
            ...
            currentLhs.set(attr);
            collectLevel(node.children[attr], currentLhs, currentDepth + 1, targetDepth, out);
            currentLhs.clear(attr);
        }
    }

    private void collectResults(FDTreeNode node, BitSet lhs, int targetLevel, int currentDepth, Map<Integer, List<BitSet>> out) {
        if (currentDepth == targetLevel) {

            for (int rhs = node.fds.nextSetBit(0); rhs >= 0; rhs = node.fds.nextSetBit(rhs + 1)) {
                out.computeIfAbsent(rhs, k -> new ArrayList<>()).add((BitSet) lhs.clone());
            }
    ...
    private void collectGeneralizations(FDTreeNode node, BitSet lhs, int rhs, BitSet current, List<BitSet> out) {
        if (node.fds.get(rhs)) {
    ...
    private boolean checkGen(FDTreeNode node, BitSet lhs, int rhs, int from) {
        ...
        for (int attr = lhs.nextSetBit(from); attr >= 0; attr = lhs.nextSetBit(attr + 1)) {
            if (node.children[attr] != null && checkGen(node.children[attr], lhs, rhs, attr + 1)) {
                return true;
            }
        }
        return false;
